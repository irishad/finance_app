class ReportsController < ApplicationController
  ...
  def result_by_category
    if params[:start_date].present? && params[:end_date].present?
      ...
      if start_date > end_date
        redirect_to reports_report_by_category_path, notice: "Дата початку періоду не може бути більшою за дату кінця періоду."
        return
      end
    end
  ...
  def result_by_dates
    if params[:start_date].present? && params[:end_date].present?
      ...
      end_date = params[:end_date]
      if start_date > end_date
        redirect_to reports_report_by_dates_path, notice: "Дата початку періоду не може бути більшою за дату кінця періоду."
        return
      end
    end
  ...
  def result_by_wallets
    if params[:start_date].present? && params[:end_date].present?
      ...
      end_date = params[:end_date]
      if start_date > end_date
        redirect_to reports_report_by_wallets_path, notice: "Дата початку періоду не може бути більшою за дату кінця періоду."
        return
      end
    end
